...
fun String.isPlate(): Boolean {
     ...
     */
    val isNewFuel = this.length == 8

    val fuelPattern = "[\\u4e00-\\u9fa5][a-zA-Z](([DF]([a-hj-np-zA-HJ-NP-Z0-9])[0-9]{4})|([0-9]{5}[DF]))"
    val normalPattern = "^[冀豫云辽黑湘皖鲁苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼渝京津沪新京军空海北沈兰济南广成使领]{1}[a-zA-Z0-9]{5}[a-zA-Z0-9挂学警港澳领]{1}$"

    val pattern = if (isNewFuel) {
        fuelPattern
    } else {
        normalPattern
    }
